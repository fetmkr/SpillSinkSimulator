...
- **깊이는 각도가 정한다 (2026-09-18).** 바닥판을 직접 못 보게 하는 깊이 = **셀 ÷ tan(입사각)**.
  셀 10 이면 45° 10.0 · 65° 4.7 · 75° 2.7 mm. 재보니 문턱이 그 자리에 그대로 있다
  (`results/depth_vs_angle_2026_09_18.json`). **45 도 너머만 쓰면 깊이 10 mm 가 깊이 20 mm 와 같다**
  (45° 0.229 대 0.228). 두께 22 → 12 mm, 칠할 면적 9.0 → 5.0 배.
  **총량만 쟀다. 얕은 깊이의 뭉개기·반짝임은 미측정. 결론을 바꾸는 것은 사용자 결정.**
