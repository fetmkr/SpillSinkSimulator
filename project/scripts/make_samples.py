...
def main():
    ...
    if "flat" in rough and "B_CONE_p0550" in rough:
        A("| 거칠기 (roughness) | 콘의 정면 밝기가 맨 검정벽 대비 25 배 변함 "
          "| ① 의 정면 반짝임 |")
    A("")
    # 적분구가 총량만 준다고 적어 놨었다. 그건 너무 세다: 정반사 자리에
    # 검은 마개를 끼우면(SCE) 확산만 나오고, 흰 마개로 잰 총량(SCI)에서
    # 빼면 정반사가 나온다. 적분구가 못 내는 것은 덩어리의 폭이다.
    A("**BRDF 로 재야 합니다.** 여러 입사각의 반사율만으로는 광택 덩어리의 "
      "폭(거칠기 α)을 낼 수 없습니다 — 이 프로젝트가 그것 때문에 여기까지 "
      "왔습니다. 장비는 고니오미터 (goniometer) 입니다. 적분구 (integrating "
      "sphere) 로도 확산과 정반사의 **비율**은 가를 수 있습니다: 정반사가 "
      "나가는 자리에 검은 마개를 끼워 재고(SCE) 흰 마개로 잰 총량(SCI)에서 "
      "빼면 됩니다. 다만 그 구멍 크기가 고정이라 **덩어리의 폭은 못 냅니다.**")
    A("")
    A("## 아직 정리 못 한 것")
    A("")
