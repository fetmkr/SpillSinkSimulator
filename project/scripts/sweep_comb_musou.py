...
OUT = "/tmp/simsrv/comb_musou"
os.makedirs(OUT, exist_ok=True)
JSONF = os.path.join(OUT, "comb_musou.json")
...
PANEL = 200.0
FOIL = 0.08
BASE = "wall_5pct"      # 5 % 무광 검정, 확산 0.97 (실측)
SPP = 256
...
for pitch in PITCHES:
    for depth in DEPTHS:
        for mus in MUSOU:
            ...
            planes = SS.measure(spec, THETAS, None, 0.30, SPP,
            ...
            f = SS.form(patch, thetas=[0.0], n_phase=6, samples=SPP,
                        beam_w=7.5, diffuse_frac=None, **kw)
            pk = f.get("peak")
