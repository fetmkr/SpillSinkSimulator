...
화면 요청 대 보고서 호출 14/14 같음. 켜 둔 서버로 14 행을 다시 렌더하는 중 (첫 5 행 최대 차 0.072 %).
같은 씨앗이라 경로 확인일 뿐이어서, 사용자 규칙으로 **무작위 씨앗 2 개를 더 잰다** (`measure_finalists_seeds.py`,
서버·화면에 `cycles_seed` 칸). 기존 결과·보고서·순위 규칙은 그대로 둔다.
