...
## 2026-09-24 — 방법 문서에 측정 규격을 넣고, 친구에게 줄 묶음(한·영)을 올렸다

**방법 문서(`report/METHOD.html`)에 규격이 빠져 있었다.** 2026-08-28 에 "빔 자리마다 재서
평균을 낸다" 를 광택 규격을 보고 정했는데, 문서에는 규칙만 남고 근거 규격이 없었다.
참고 문헌 표 아래에 **측정 규격** 표를 달았다: ASTM D523-89(1999), ASTM E430-97,
ISO 2813:1994 (미리보기), NPL 좋은 측정 안내서 37 호, Talbot–Plateau 법칙 (Wikipedia).
문헌 표에 Hsia 1976 (NBS TN 594-12, 판 크기는 빛이 옆으로 걷는 거리로) 과
Schneider FDTD 교재를 더했다. 인용문은 전부 파일에서 글자 그대로 찾아 확인했다.

**새로 드러난 문제 둘을 "아직 근거가 없는 것" 에 적었다.**
- 광택 규격은 읽는 창을 **각도로** 정한다. 우리 창(60 × 40)은 아직 아니다.
- 평균이 맞다는 근거(Talbot–Plateau)는 레이저가 눈의 문턱보다 빨리 훑을 때만 선다.
  문턱은 막대세포 약 15 Hz, 원뿔세포는 아주 밝을 때 약 60 Hz. **우리 레이저의 화면
  주사율은 안 쟀다.** 8/28 에 적은 "20~30 Hz" 는 잰 값이 아니다.

**원문 파일.** `reference/papers_method/` 에 빠졌던 PDF 12 개를 받았다 (Efron, Forde,
Kriegeskorte, Pawlus, Zirr, Kanit, Veach 전체, MCNP, NBS Mono 160, IEA Task 61, Hsia,
ISO 2813:1994 미리보기) + Wikipedia 쪽 PDF. `veach_chapter3.pdf` 는 404 HTML 이라 지웠다.
PDF 는 `.gitignore` 라 이 맥에만 있다. ASTM 둘은 글자만 뽑은 .txt 이고 파일 공유 사이트
(kelid1.ir)에서 받은 것이다 — 사용자가 알고 공유하기로 했다.

**문서마다 파일 칸.** 제목은 원문 주소, 파일 칸은 받아 둔 사본으로 건다.

**영어판.** `scripts/method_en.py` 에 글만 있고, 숫자는 한국어판처럼 코드에서 읽는다.
한국어 쪽 줄 수·설정 순서·문헌이 영어 쪽과 어긋나면 빌더가 멈춘다. 한국어판 출력은
바꾸기 전과 바이트 단위로 같다 (`cmp`).

    python3 scripts/build_method_report.py                        # report/METHOD.html
    python3 scripts/build_method_report.py --lang en              # report/METHOD_en.html
    python3 scripts/build_method_report.py [--lang en] --bundle <폴더>   # 아티팩트 묶음

묶음은 `<폴더>/index.html` + `<폴더>/papers/` 20 개. 15 MB 넘는 MCNP 매뉴얼(20 MB)은
아티팩트 파일 한도 때문에 묶음에서 빼고 원문 주소로 건다.

**아티팩트 (비공개, 공유는 사용자가 Share 메뉴로).**
- 한국어 https://claude.ai/artifact/MUaEsSVXkfBdgWNsh4rjLS
- 영어 https://claude.ai/artifact/X9g8gH9C2GCyGYB7EfdZzE (문헌 파일은 한국어판에서 서버 복사)

---

## 2026-09-18 (이어서) — 깊이를 정하는 것은 각도다. 발주 제안 정정

**셀 깊이는 빔이 들어오는 각도가 정한다.** 천장 3.5 m 로 각도가 45~78 도가 되니
깊이 20 mm 가 필요 없다. 기하와 측정이 같은 자리를 가리킨다.

바닥판을 직접 못 보게 하는 깊이 = **셀 ÷ tan(입사각)**. 셀 10 mm 면
30° 17.3 · 45° 10.0 · 55° 7.0 · 60° 5.8 · 65° 4.7 · 70° 3.6 · 75° 2.7 mm.

재봤다 (`results/depth_vs_angle_2026_09_18.json`, `scripts/depth_vs_angle_2026_09_18.py`,
셀 10 · 전부 무소 · 판 500 · 빛줄기 256, 1 분):

| 깊이 | 0° | 30° | 45° | 55° | 60° | 65° | 70° | 75° |
|---|---|---|---|---|---|---|---|---|
| 20 mm | **0.075 %** | **0.174 %** | **0.227 %** | **0.273 %** | **0.303 %** | **0.335 %** | **0.374 %** | **0.421 %** |
| 15 mm | 0.109 % | 0.179 % | **0.228 %** | **0.273 %** | **0.302 %** | **0.335 %** | **0.374 %** | **0.420 %** |
| **10 mm** | 0.183 % | 0.212 % | **0.229 %** | **0.274 %** | **0.302 %** | **0.335 %** | **0.375 %** | **0.420 %** |
| 6 mm | 0.319 % | 0.315 % | 0.289 % | 0.287 % | **0.303 %** | **0.335 %** | **0.375 %** | **0.422 %** |
| 4 mm | 0.444 % | 0.439 % | 0.410 % | 0.379 % | 0.363 % | 0.353 % | **0.377 %** | **0.422 %** |
| 3 mm | 0.531 % | 0.534 % | 0.519 % | 0.495 % | 0.472 % | 0.439 % | 0.409 % | **0.423 %** |
| 2 mm | 0.642 % | 0.661 % | 0.679 % | 0.691 % | 0.686 % | 0.666 % | 0.614 % | 0.514 % |

굵은 칸이 깊이 20 과 2 % 안에서 같아진 곳이다. **문턱이 셈이 말한 자리에 그대로 있다**
(45° 10 · 60° 6 · 70° 4 · 75° 3 mm).

**뜻하는 것.** 빔이 45 도보다 비스듬히 맞는 자리만 쓰면 **깊이 10 mm 가 깊이 20 mm 와 같다.**
판 두께 22 → 12 mm, 칠할 면적 판 면적의 9.0 → **5.0 배** (식 4d/p+1).
지은 메시 배수는 안 쟀다 — 깊이 20 은 15.7 배였고 벽 몫만 깊이에 비례해 준다 [추측 9 배 안팎].
**조건:** 45 도보다 곧게 맞는 자리가 있으면 안 된다. 30 도에서 깊이 10 은 깊이 20 보다 22 % 밝다.
그 자리는 프로젝터 바로 위 반지름 1.5 m 안이다.
**아직 안 잰 것: 얕은 깊이의 뭉개기·반짝임.** 총량만 쟀다. 결론을 깊이 10 으로 바꾸는 것은 사용자 결정.

**연구 기록 문서를 다시 엮었다** (아티팩트 v14).
- 맨 뒤 "덧붙임 · 9월 16일" 을 **05 절 "깊이를 정하는 것은 각도다"** 로 앞당겼다 (사용자 지시:
  "가격 정할 때 허니콤 셀 쓰는 게 말이 안 되잖아" — 값 매기는 절보다 앞에 와야 한다).
  절 번호가 밀렸다: 직접 해보기 05→06, 돈 06→07, 남은 것 07→08. 링크는 문서에 없어 깨진 곳 없음.
- 절 안 차례: 하나 깊이 40 불필요 → **둘 각도가 깊이를 정한다(새로)** → 셋 셀 안쪽 안 칠하기
  → 넷 셀 크기 → 다섯 형상 코드 결함.
- "견적을 넣어보는 칸" 에 "깊이 칸을 10 으로 내려 보라" 한 줄을 넣었다.

**발주 제안 문서를 저장소로 가져오고 정정했다** (`report/comb/order_proposal_2026-09-16.html`,
아티팩트 https://claude.ai/artifact/HSPZ9Ug7Uv1RGqP8n5ecNP v2). 그 문서에는 5.02 배가 없었다.
대신 **2026-09-16 판이라 아직 피라미드를 제안하고 천장을 6 m 로 그린다.** 본문은 그대로 두고
맨 앞에 정정 상자를, 3·5·8·9·12 절에 정정 줄을 넣었다: 결론이 벌집으로 바뀜, 천장 3.5 m,
스치는 각 총량 표(75° 피라미드 0.690 % 대 벌집 0.422 %), 칠할 면적 배수(피라미드 12.15 배는 맞고
벌집은 식 9.0 배·메시 15.7 배). **본문을 벌집으로 다시 쓸지는 사용자 결정 대기.**

---

## 2026-09-18 — 방 조건이 바뀌었다 (천장 3.5 m). 연구 기록을 인터랙티브로, 화면 정리 시작

**천장이 6 m 가 아니라 3.5 m 다** (사용자가 알려줌). 이 값이 연구 전체의 각도 조건을 흔든다.

**1. 각도 계산.** 프로젝터 2 m 높이 기준, 45 도가 되는 수평 거리가 천장 6 m 에서는 4.0 m, **3.5 m 에서는 1.5 m** 다.
그보다 먼 천장은 전부 45 도를 넘는다. **우리는 0~45 도만 쟀다.**

| 수평 거리 | 천장 6 m | 천장 3.5 m |
|---|---|---|
| 1 m | 14.0° | 33.7° |
| 1.5 m | 20.6° | **45.0°** |
| 3 m | 36.9° | 63.4° |
...
**2. "정면이 없어지니 낫지 않나" 는 반대다.** 정면이 우리 설계에서 가장 어두운 각도였다.
셀 10 벌집: 0° 0.075 %, 30° 0.174 %, 45° 0.227 %. 무소 민판: 45° 1.219 %, 60° 1.586 %, 75° 2.455 %.
**낮은 천장은 어두운 각도를 없애고 밝은 각도만 남긴다. 45 도 너머는 아직 안 쟀다.**

**3. 인터랙티브 도구 둘.**
- 단독 도구 `report/comb/beam_angle_2026-09-18.html` (아티팩트 https://claude.ai/artifact/7pE1t8FBJ4ZKmjYeQNNUz8)
- **연구 기록 00 절**에 같은 계산을 넣었다. 화면(공중 영상) 개념은 사용자 요청으로 뺐고,
  **빔 가운데가 천장의 정한 자리를 겨누고, 프로젝터는 좌우 대칭으로 움직이며, 겹치는 구간(크로스)을 표시**한다.
  바닥 좌표는 -5 ~ 0 ~ +5 m.

**4. 연구 기록 문서를 저장소로 가져왔다.** 전에는 아티팩트에만 있었다.
`report/journey/record_ko_2026-09-18.html` (아티팩트 https://claude.ai/artifact/3gPYCx3j6N2CANw7BRB8Bm, v12).
같이 한 것: 레이저 사진 추가(00 절), 한글 폰트 **Pretendard**(문서와 같이 올림, SIL OFL),
**색을 시뮬레이터 팔레트로** (`sim/index.html` :root 값), 밝은 화면 하나로 고정, 용어 영어 병기,
맨 위 한/영 단추. 그림 8 장도 아티팩트에서 내려받아 저장소에 넣었다.

**5. 시뮬레이터 화면 정리 (사용자 지시: 1·2·6 먼저, 4 는 보류).**
- 조건은 `method` 칸 한 곳에만. 각도 목록(총량 θ·φ, 뭉개기 빔, 순위 각도)을 여기에 더했다.
- 단추 밑 설명을 한 줄로 줄이고 나머지는 접었다.
- 이름 통일: "관객 각도 observer", φ 는 "방위".
- 로고: FETM 옆에 **Kimchi and Chips** 로고, `fetm.kr` 글씨 제거, 칸막이 선과 여백 정리.
  FETM 로고는 원본(흰 배경) 그대로 둔다 (사용자 지시).

**6. 스치는 각도를 잴 수 있게 하고, 쟀다.** 화면에 **"스치는 각도까지 재기"** 칸을 넣었다 (±55·65·75°).
규약(`form_metrics`)은 그대로 두고 요청에만 각도를 더한다. 꺼 두면 옛 요청 그대로라 화면 요청 검사 14/14 유지.
총량 6 → 12 초. 광선 추적도 같은 목록을 쓴다. 결과 `results/grazing_2026_09_18.json`.

| 설계 | 0° | 45° | 75° | 75/45 |
|---|---|---|---|---|
| 5 % 민판 (거의 완전 확산) | 5.000 % | 5.013 % | 5.252 % | **1.05 배** |
| 무소 민판 (광택 있음) | 0.967 % | 1.219 % | 2.455 % | **2.01 배** |
| 벌집 셀 10 (결론) | 0.075 % | 0.227 % | 0.421 % | 1.85 배 |
| 벌집 셀 20 | 0.176 % | 0.219 % | 0.399 % | 1.82 배 |
| 발주 피라미드 | 0.164 % | 0.244 % | 0.365 % (방위 0 은 0.689 %) | 1.50 배 |

**왜 다 밝아지나 — 도료의 프레넬이다.** 거의 완전 확산인 5 % 판은 1.05 배로 거의 안 변하고,
광택이 있는 무소는 2.01 배가 된다. 구조가 무너진 것이 아니라 **도료가 오른 만큼 따라 올랐다**
(벌집 1.85 배는 도료 2.01 배보다 낮다). 벌집은 75° 에서도 무소 민판보다 5.8 배 어둡다.
**다만 75° 근처는 실물로 확인한 적이 없다.** 논문 그림에서 읽은 값에 맞춘 재료다.

**7. 각도 규칙 (발표에서 바로 쓸 수 있다).**
빔이 닿는 천장 구간이 **프로젝터 바로 위 ±(천장 높이 − 프로젝터 높이)** 안이면 전부 45° 안쪽이다
(천장 3.5 · 프로젝터 1.2 이면 ±2.3 m). 낮추거나 가까이 겨누면 넓어진다.
**판을 기울이면 기울인 만큼 입사각이 준다** — 75° 자리에 30° 기울이면 45° 값(0.227 %)이 된다.
사용자 판단: **기울기 칸은 안 만든다. 지금 각도 표로 발표 때 예측할 수 있다.**

**못 한 것.** 영어판 `record_en_2026-09-18.html` 은 **머리말과 00 절까지만 번역**했다 (아래 NEXT 참고).
스치는 빔(55·65·75°)에서의 뭉개기·반짝임. 시뮬레이터 정리 3·5. PDF 보고서 발치의 로고·`fetm.kr`.

---

## 2026-09-17 — 시연 준비. 화면을 순서대로 직접 눌러 보고 틀린 곳을 고쳤다

**계기.** 연구자 대상 30 분 시연 (사용자 결정: **결론은 벌집**, 셀 10 / 깊이 20).
사용자가 화면에서 이상한 값(0.297 %, 0.357 %)을 보고 "시연 순서대로 직접 해 보고
오류를 찾아라" 고 했다.

**1. 측정값은 틀린 것이 없었다.** 화면에서 누른 값이 보고서·서버 직접 측정과 같다.

| 단계 | 화면 값 | 대조 | 걸린 시간 |
|---|---|---|---|
| 5 % 민판 (판 200) 총량 | 5.01287 % | 보고서 5.013 | 6 초 |
| 무소 민판 (판 200) 총량 | 1.21860 % | 보고서 1.219 | 6 초 |
| 벌집 셀 10 / 깊이 20 / 판 500, 무소 전부 총량 | 0.22745 % (45 도) | 서버 직접 0.22745 | 7 초 |
| 광선 추적 (7 각도) | 정상 | — | 7 초 |
| 표준 샘플 피라미드 판 60 뭉개기·반짝임 (관객 0) | 1.852 · 0.0775 | 보고서 1.8516 · 0.0775 | 48 초 |

**2. 이상한 값의 원인은 바닥판 재료였다.** 셀 10 은 바닥판이 곧게 보인다 (0 도).
무소 칠 100 % 벌집에서 바닥판만 바꾸면 0 도 값이 무소 0.075 %, 양극산화 0.297 %,
아크릴 0.356 %. **방 조건(30~45 도)은 셋 다 0.228 % 로 같다.** 화면 큰 숫자는 전체
각도 최대라 0 도 값이 떴다.
- 원인 하나: bare 칸에서 고른 재료가 바닥판 "같게" 를 타고 바닥판으로 들어갔다 (고침).
- 원인 둘: 브라우저에 저장된 설계 "final" 의 바닥판이 아크릴로 직접 저장돼 있다
  (localStorage `sim_user_designs`). **사용자 데이터라 안 건드렸다. 다시 저장해야 한다.**

**3. `sim/index.html` 에서 고친 것** (측정 요청은 안 바뀜, 화면 요청 검사 14/14 그대로)
- bare 칸이 칠 100 % 에서도 고른 재료를 보여준다 ("칠이 100 % 덮어서 결과에 안 들어간다").
  잠그자는 내 제안은 사용자가 거절했다.
- 바닥판 "— same as the top layer —" 가 **보이는 위층**을 따른다 (칠 100 % 면 칠 재료).
  `gate_finalists_ui_path.py` 의 옮긴 코드도 같이 고쳤다.
- 칠 비율을 숫자 칸에 입력해도 재료 카드가 갱신된다.
- 결과 밑 재질 줄이 `slotMaterials()` 를 읽는다. 5 % 민판을 "전체 musou_fit2" 로,
  확산·거칠기를 패널 슬라이더(0.00/0.60)로 적던 결함.
- 단추 설명: 총량 "7 angles × 2 planes", 뭉개기 "16 stripe positions, 1~4 min",
  "보고서보다 정밀도 낮음" 문구 삭제 (틀린 말이었다).
- 결과 표 `.scroll` 이 정의가 없어 45 도 칸이 잘렸다. 그래프 가로축이 -40~40 고정이라
  45 도가 밖으로 나갔다. "가장 나쁜 쪽" → "가장 밝은 쪽".
- 세 축 칸 기준 문구를 오늘 잰 값으로 (무소 민판 1.14 → 1.22 %).
- **뭉개기·반짝임 결과에 관객 각도를 서버 응답(`obs_elev_deg`)에서 찍는다.** 입력칸만
  있고 결과에는 안 나와서, 결과를 보고 몇 도에서 잰 값인지 알 수 없었다.
- **광선 추적 각도를 `/api/protocol` 의 총량 각도로** (0, ±20, 30, ±40, 45). 30·45 도가
  없었다. 색 30 도 연두, 45 도 노랑 (둘이 조금 비슷하다).

**4. 보고서 기준 줄 둘** (`report/comb/finalists_2026-09-15.html`, 순위 밖, 아티팩트 v6)

| | 총량 | 반짝임 | 뭉개기 |
|---|---|---|---|
| 무소 민판 | 1.219 % | 0.459 (빔 -40 / 관객 60) | 1.00 |
| 5 % 민판 | 5.013 % | 4.190 (빔 -40 / 관객 40, 거울 방향) | 1.00 |

`measure_finalists.py` `REF_CASES` → 결과 파일의 `reference` 키. `rows` 14 개는 그대로.

**5. 셀 10 은 comb 결함에 안 걸린다.** pitch 10 과 10.0000001 은 메시가 다르다
(판 500 깊이 20 에서 꼭짓점 103,096 대 101,424) **하지만 방 조건 총량은 0.2275 대
0.2271, 0.16 %.** 9.53 은 같은 흔들기로 14 %. 가장자리 칸 수 경계로 보인다 [추측].

**6. 읽는 창 60 × 40 에는 근거가 없다** (`form_metrics.py` 34 줄에 그렇게 적혀 있다).
실측 근거는 "가장자리에 딱 붙이면 배경이 섞인다" 하나 (`FINDINGS_rig_audit_2026_08_20`
100 줄, 5 % 판이 0.0530). 사용자 요청으로 **50 × 50** 을 같은 프로세스에서 견줬다
(`scripts/window_50x50.py`, `results/window_50x50_2026_09_17.json`).

| | 60 × 40 | 50 × 50 |
|---|---|---|
| 총량 방 조건: 결론 벌집 셀 10 | 0.22745 | 0.22766 (+0.09 %) |
| 벌집 9.53/40 | 0.20842 | 0.20726 (-0.56 %) |
| 발주 피라미드 | 0.24382 | 0.24382 |
| 표준 샘플 | 0.24404 | 0.24688 (+1.16 %) |
| 반짝임 순위 값: 발주 / 표준 | 0.1331 / 0.1355 | 0.1331 / 0.1355 |

**순위 값은 거의 안 바뀐다. 작은 값 칸은 흔들린다** (총량 0 도 최대 6 %, 반짝임 빔 -40
최대 30 %). 두 창 비교로는 어느 쪽이 맞는지 못 정한다. 결론 벌집은 아래 8 번.

**7. 시연에서 조심할 것.** 서버는 한 번에 하나만 처리한다 (`on_main` 큐, 취소 없음).
셀 20 으로 바꾸면 큰 숫자가 0.219 % 로 **작아진다** (45 도만 큰 셀이 이김, 0 도는 2.3 배
밝음). 화면 반짝임 칸은 빔 0 도·관객 한 자리 값이고 보고서 순위 값과 다르다.
광선 추적 숫자는 줄기가 적어 인용 불가 (30 도 out 0.257 % 대 렌더 0.162 %).

**8. (같은 날 이어서) 진행표와 시연 설계.**
- 진행표 `report/comb/demo_runsheet_2026-09-17.html`
  (아티팩트 https://claude.ai/artifact/H6ywiNQ91g7MhCd337zhNR).
- 화면 설계 목록 맨 위에 **"시연 (진행표 순서)"** 여섯 개 (`sim/index.html` `DEMO`). 브라우저 저장소가 아니라
  파일에 있어서 어느 브라우저든 새로고침하면 뜬다. 여섯 다 불러와서 눌러 확인: 5.01287 · 1.21860 ·
  0.22745 (판 500) · 0.21853 · (피라미드 뭉개기 설정 같음) · 0.24382.
- **설계를 불러오면 판 크기 슬라이더가 200 에서 잘리던 결함을 고쳤다.** 범위 입력은 최대값에서 멈춘다.
  숫자 칸에 치는 길은 범위를 넓혔는데 불러오는 길(`applySpec` 의 `setTop`)은 안 넓혔다. 판 500 설계가
  200 으로 재졌다 (셀 10 이 0.22785 로 나와서 알았다). 형상 매개변수 슬라이더도 같이 고쳤다.
- **결론 벌집 세 축이 찼다** (판 500, 60×40 창): 총량 0.227 %, **반짝임 0.1453** (빔 +40 / 관객 20),
  **뭉개기 1.098 / 1.027 / 1.045** (관객 20/40/60, 관객 0 은 안 잼). 관객 한 자리 11~18 분.
  50×50 창으로 바꿔도 모든 칸 0.18 % 안.

**못 한 것.** 셀 10 벌집의 관객 0 뭉개기 (보고서 뭉개기 순위 기준). 셀 10 벌집의 뭉개기·반짝임 (판 500 관객 한 자리 10 분 넘음,
켜 둔 서버로 보냈다가 사용자 화면을 막아서 멈췄다 — `demo_comb_p10_d20_2026_09_17.json`
은 총량만). 세 축 칸 색 기준(총량 0.20 %, 반짝임 0.06) 을 둘지 사용자 결정 대기.

---

## 2026-09-16 — 벌집을 다시 봤다. 셋은 설계를 바꾸고, 하나는 발표된 숫자를 흔든다

정본: `results/FINDINGS_comb_alu_cell_2026_09_16.md`.
보고서 8 절 "덧붙임 · 9월 16일" (아티팩트 https://claude.ai/artifact/3gPYCx3j6N2CANw7BRB8Bm).

**계기.** 사용자 질문 넷. 은색 셀 내부, 깊이 20, 셀 크기 훑기, 방위 돌리기.

**1. 셀 안쪽을 은색으로 둬도 된다.** 위 20 mm 만 무소로 칠하면 세 축이 전부 칠한 것과
같다 (총량 0.166 %, 뭉개기 1.014, 반짝임 0.1215 / 기준 0.166 · 1.013 · 0.1214).
테두리만 칠하면 총량 30 %, 반짝임 18.41 로 무너진다. **빔이 4~23 도로 들어오면
3.5 배 밝아진다** (첫 벽 닿는 깊이가 21~25 mm 라 칠한 20 mm 를 지나침). 이 방은
30~45 도라 안전하고 여유 7 도. 30 mm 칠하면 그 구간이 사라진다.
맨 알루미늄 재료 `material/alu_lambert.json` (총반사 0.86, Pozzobon 2019 적분구 실측).

**2. 깊이 40 은 필요 없다.** 방 조건에서 깊이 20 과 **6.7 % 차이**다 (0.208 → 0.215 %).
깊이 40 을 고른 근거가 정면(0.039 대 0.080)이었고 **그 축은 이틀 뒤 2026-08-27 에
폐기됐는데 깊이 결정을 다시 안 봤다.** 깊이 20 전부 칠함이 깊이 40 반만 칠함을 이긴다
(도료량 같고, 코어 절반, 두께 42 → 22 mm, 각도 금지 구간 없음).

**3. 셀은 작은 쪽이 안전하다.** 45 도 한 칸만 큰 셀이 이기고 나머지 각도는 다 진다
(10 도에서 1.9 배). 큰 셀은 바닥판이 그대로 보인다 — 바닥 재료를 바꾸면 30 도가
셀 6.5 에서 +0.1 %, 셀 20 에서 **+135.7 %** 움직인다. **셀 10 / 깊이 20 으로 정했다**
(사용자 결정). 칠할 면적 **9.0 배** (식 4d/p+1), 실제 지은 메시로는 **15.7 배**, 두께 22 mm, 방 조건 약 0.23 %.
(2026-09-18 정정: 그때 적은 5.02 배는 근거가 없고 식·메시 어느 쪽과도 안 맞았다.)

**4. `comb` 형상 코드가 pitch 정확히 9.53 과 6.35 에서 다른 격자를 짓는다.**
1e-7 mm 를 바꾸면 벽이 12 % 늘고 총량이 14 % 어두워진다. 바깥 크기는 같다.
6.5 와 12.7 은 멀쩡하다. **발표된 벌집 총량이 9~10 % 움직인다**
(벌집 9.53/40 0.208 → 0.2285, 전체 2 위 조합 0.197 → 0.2149). 총량 순위가 바뀐다
(원뿔 0.223 에게 진다). **어느 쪽이 맞는지 아직 모른다.** `_build_comb` 주석에 실제
벌집의 두 겹 벽을 모델링한다고 적혀 있어, 9.53 만 제대로 짓는 것일 수도 있다.
증거는 "9.53 이 틀렸다" 쪽 — 멀쩡한 다섯 점이 매끄러운 줄인데 의심 두 값만 벗어난다.
**풀리기 전에는 벌집 숫자를 확정으로 쓰지 않는다.**

**방위.** 육각은 60 도 되풀이라 0 도(평평한 면)와 30/90 도(꼭짓점)가 양 끝이다.
규약의 방위 45 는 한가운데(15 도와 같음)라 꼭짓점 끝을 안 본다. **놓치는 몫은 1 % 안**
이라 규약은 그대로 둔다. 다만 **판을 어느 방향으로 까느냐가 8 % 를 정한다.**

**못 한 것.** 시뮬레이터에 금속 모델이 없다 (`blender_render.py` 327 줄 `Metallic 0.0`,
유전체 프레넬이라 정면 정반사 최대 4 %, 총반사 86 % 는 확산 0.953 이상만 표현 가능).
거울 쪽 알루미늄을 못 잰다. 셀 10/깊이 20 의 뭉개기·반짝임 미측정. 실물 측정 여전히 0 건.

---

## 2026-09-14 ~ 15 — 시뮬레이터 감사 다섯 항목을 고치고, 최종 후보 14 개를 다시 쟀다

정본: `results/FINDINGS_simulator_audit_2026_09_14.md` (조치 1~11, 다음 세션 시작점).
보고서: `report/comb/finalists_2026-09-15.html`. 커밋 `f543c04`, `855af32`, 그리고 이 절을 담은 커밋.

**왜 했나.** 외부 감사가 시뮬레이터 결함 다섯 개를 짚었다. 이 결함들이 그동안 발표한
숫자를 흔들 수 있어서, 새 형상을 찾기 전에 측정 장비부터 고쳤다.

**무엇을 고쳤나.**
1. **요청 전달이 두 갈래였다.** Blender 안 서버와 일반 Python 서버가 요청을 따로 풀어서,
   일반 서버에서는 코팅·도장 깊이·관찰자 각 등 열 개가 측정 함수에 안 닿았다. `run_op` 하나로 합쳤다.
   모르는 키는 `ignored_keys` 로 돌려준다. 검사 `gate_dispatch_equivalence` 13/13, 실제 렌더 `gate_dispatch_render` 6/6.
2. **코팅이 빛의 방향을 바꾸면 다른 값을 냈다 (상반성 위반).** 옛 트리는 광원과 눈을 바꾸면 238 % 달랐다.
   확산 + 유전체 GGX 를 상수 비율로 섞는 트리로 바꿨고 차이 0.001 %. `gate_coating_reciprocity` 6/6.
3. **무소 재료를 논문 그림에서 잘못 읽었다.** Filip & Vávra 그림 5·6 을 다시 읽어 맞춘 `musou_fit2`
   (확산 0, 광택 0.50, 거칠기 0.86). 기본 재료가 됐다. 거칠기 0.45~0.75, 절대 크기 ±20 % 는 열려 있다.
4. **반짝임 통계를 화소 하나(p99/max)로 냈다.** 2 mm 상자 평균의 최대(box)로 바꿨다. `gate_form_stats` 9/9.
5. **뭉개기가 수렴하지 않았는데 수렴했다고 했다.** 가장 넓은 창 + 가장자리 빛 검사로 판정한다.
   판 밖으로 나간 빛은 여전히 못 본다.

**고치다 나온 것.** 대조판을 형상이 덮어 회귀 잠금이 0.044 로 흔들렸다 (필드 끝 뒤로 옮김).
빛줄기 16 은 box 에 모자랐다 (벌집 정면 +4.55 %) → 256. 옛 목표 0.040 은 옛 통계라 비교 불가,
같은 표준 샘플이 지금 기준 0.077. 광선 그림도 새 재료 모델로 샘플링한다.

**최종 후보 14 개.** 1,643 개 전체 재산출 대신 사용자와 합의한 구조·도장 조합만 쟀다.
전체 1 위 발주 피라미드 4/22 끝 20 % 무소 (총량 0.244 %, 반짝임 0.133, 뭉개기 2.14),
2 위 벌집 테두리 0.03 + 피라미드 바닥 15, 3 위 표준 샘플 4/20. 무소 끝 칠은 반짝임을 5~6 배 낮춘다.
뒤집힌 피라미드는 뭉개기가 1 근처로 무너진다.

**시뮬레이터도 같은 기준으로.** 사용자가 보고서 값을 화면에서 직접 재현하기로 해서, 화면 각도·방위·빛줄기를
`form_metrics` 한 곳에서 받게 하고 `/api/form` 이 요청한 판 크기 값을 먼저 돌려주게 했다.
화면 요청 대 보고서 호출 14/14 같음. 켜 둔 서버로 실제로 다시 렌더한 5 행은 보고서와 최대 0.072 % 차.
사용자 지시로 전부 하지 않고, 남은 9 개에서 무작위로 뽑은 2 개만 더 재고 끝냈다. V 홈 13 은 모든 칸 최대 차 0.003 %,
밑변 50/250 은 총량 14 칸만 최대 차 0.00001 % (뭉개기·반짝임은 판을 키워 다시 재느라 몇 시간이라, 같은 값이 나온 데서 멈춤).
보고서 "시뮬레이터 검증" 절에 "시뮬레이터도 잘 검증됐다" 로 적었다.
기존 결과·보고서·순위 규칙은 그대로 둔다.

**실측 없이는 모르는 것.** 무소 재료값, 스치는 각의 좁은 봉우리, 2 mm 상자가 눈부심과 맞는지, 판 밖 꼬리, 파장별 값.

---

## 2026-08-27 — 방이 정해지니 답이 바뀌었다

보고서: `report/comb/room_and_observer_2026-08-27.html` (영문판 `_en`)
훑기: `scripts/sweep_observer_scan.py` → `results/comb20/observer_scan_both.json`

**설치 조건 (사용자 확정).** 방 10 x 10 m, 천장 6 m, 프로젝터 높이 2 m 에서
수평보다 45~60 도 위로. 패널은 천장. 그러면 패널이 받는 각도가 30~45 도이고
**정면은 이 방에서 안 일어난다.** 정면이 되려면 프로젝터가 천장 바로 밑에
있어야 하는데 그러면 공중 이미지가 안 생긴다.

**그런데 이 프로젝트가 발표한 봉우리는 전부 판 법선 방향 값이었다.**
`setup_camera` 는 처음부터 각도를 받았는데 `run_case` 가 0.0 을 글자 그대로
넘겼다. 오늘 관찰자 각도를 손잡이로 만들었다 (`OBS_ELEV`, `/api/form` 의
`obs_elev`, 화면 칸, 3D 카메라 그림, `peak_by_theta`, 기울인 축 mm 환산
`1/cos`, 검사기 `gate_observer_angle.py` 4항목). 안 보내면 판 법선이라
발표된 66,426 행은 안 움직인다.

**모양은 빛을 없애지 않고 옮긴다.** 빔 40 도, 무소 팁 20 mm + 5 % 페인트,
민판 무광 검정 = 1.0:

| 관찰자 | 벌집 9.53/40 | 피라미드 50/250 |
|---|---|---|
| 0도 | 0.0070 | 0.0773 |
| 20도 | 0.1329 | 0.1001 |
| 30도 | 0.1347 | 0.1585 |
...
총량으로는 벌집이 민판보다 42 배 어두운데 빛이 온 쪽에서는 0.137 이다.
**벌집이 밝은 이유는 관이 아니라 벽이다** — 빔이 40 도로 오면 셀 한쪽 벽이
`간격/tan40` = 11.4 mm 까지 밝아지고 그 면은 빛이 온 쪽에서만 보인다.
그래서 20~60 도가 통째로 평평하게 밝다. **피라미드는 골이 되쏜다** — 밑변 50
높이 250 이면 옆면이 84 도로 서서 좁은 골이 되고 두 번 튕긴 빛이 온 길로
나간다. 높이를 올릴수록 총량은 좋아지는데 골은 좁아진다. 두 축이 반대로 간다.

**둘 다 목표 0.040 미달** (벌집 3.4 배, 피라미드 11.7 배). 그리고 한 모양이
다 이기지 않는다. **다음에 정할 것은 모양이 아니라 관객 자리다.** `NEXT.md` I절.

같은 날 고친 것 둘. `/api/measure` 와 `/api/rays` 가 버린 기본값(확산 0.76,
거칠기 0.30)을 쓰고 있었다 — 네 칸 다 고치고 `gate_api_defaults.py` 로 막았다.
보고서 짓는 스크립트는 서식이 안 풀리면 파일을 쓰기 전에 발행을 멈춘다.
...
정면으로 되돌아오는 빛을 셋으로 나누면 **포일 테두리 51.4 % · 셀 벽
1~20 mm 0.4 % · 바닥판 48.3 %.** 테두리는 판 겉면이고 바닥판은 평평한
받침판이라 벌집을 붙이기 전에 칠하면 된다. **관 안쪽 깊은 데는 정면에 대해
아무 일도 안 한다.** 비스듬한 빛만 벽을 쓰고 15 mm 에서 끝난다
(`간격/tan40 = 11.4 mm`).

정면이 40도보다 다섯 배 어두운 이유: 관은 한쪽으로만 열린 덫이다. 곧게 들어온
빛은 벽을 안 스치고 바닥까지 가지만 거기 출구가 반각 6.79 도라 `sin²` 로
1.40 % 만 나간다. 비스듬한 빛은 5.68 mm 에서 벽에 닿고 거기 출구는 40 도라
41 % 가 나간다. **손계산이 렌더와 1.9 %, 6.1 % 안에서 맞는다.**

깊이·셀·포일: **40도 총량은 깊이와 무관하고** 그게 정면보다 다섯 배 크다.
정면만 깊이가 사는데 20→60 몫의 83 % 를 40 mm 가 이미 준다. 바닥판을 칠하면
셀 순위가 뒤집혀 9.53 이 6.35 를 세 각도에서 모두 이긴다. 남은 지렛대는 깊이가
아니라 **포일 두께** 다 (0.08→0.04 가 세 각도 다 좋아지고 판 두께는 그대로).
추천: **셀 9.53 · 깊이 40 · 포일 0.03~0.04 · 바닥판 따로 칠해서 붙이기.**

사각 격자(날을 미리 칠하고 슬롯으로 조립)는 세 축 어디서도 벌집과 7 % 를
안 넘게 다르다. 차이의 원인은 모양도 구멍 크기도 아니고 **펼친 벌집의 두 겹
벽**이다 -- 구멍 넓이를 맞추면 사각과 홑겹 육각이 3 % 안에서 같다. 담그기를
못 하면 격자가 정면 3.2 배 어둡고 반짝임 6.1 배 낮다.

아노다이징은 4.5 % 로 무광 검정 도료 5.0 % 와 사실상 같아서, 못 하는 것이
손해가 아니다.

오후에 전파 흡수체 논문을 보고 두 가지를 고쳤다. 파장이 만 배 달라 저쪽
수법 대부분은 안 넘어오지만(우리 445 nm 에게 셀 9.53 mm 는 파장의 21,415 배),
"갑자기 바꾸지 말고 서서히" 라는 원리 하나는 옮겨진다. **테두리만 얇게**
(`wall_top` 이 노출 넓이를 혼자 정한다) 하고 **바닥판을 눌러 찍은 피라미드로**
바꾸니, 아침 추천 대비 정면 총량 절반 · 정면 반짝임 6.7 배 낮음.
그리고 **모양 뭉개기가 처음으로 움직였다** (0.9594 → 1.0363, 목표 1.42).

사용자가 물은 **뒤집힌 피라미드는 우리에게 안 좋다.** 총량·반짝임은 같은데
뭉개기가 1.46 에서 1.07 로 무너진다 -- 파인 홈은 직각 두 면이 코너 리플렉터가
되어 빛을 온 쪽으로 되돌려 보낸다. 태양전지에서 그게 이득인 이유가 우리에겐
손해인 이유다. `geom_floor` 에 `pyramid_inv` 로 만들어 두었다.

시뮬레이터: 만드는 방법이 모양에 묶여 있던 것을 풀었다(`spec["process"]`,
화면의 "Made how", `/api/processes`). 3D 색이 자리가 아니라 **재료**를 따라간다.
