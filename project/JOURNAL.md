...
3. **시킨 것보다 크게 벌였다.** 같은 씨앗 재현이 0.00001 % 로 맞자 사용자가 이유를 물었고, 씨앗 두 개를 더 재기로 했다.
   나는 보고서 순위 규칙·파레토·표시까지 바꿨다. "이미 한건 그대로 두고 2개만 더해보자" — 빌더를 커밋본으로 되돌렸다.
...
켜 둔 서버로 14 행을 다시 렌더하니 0.07 % 이하로 맞았다. 같은 코드·같은 조건·같은 씨앗이라 같은 빛줄기를 다시 쏜 것이다.
화면 경로가 보고서 경로와 같다는 증거이지 흔들림이 없다는 증거가 아니다. 그래서 무작위 씨앗 두 개를 더 잰다.
