...
반짝임 절대값은 시험 조각 하나를 고니오미터로 재기 전까지 못 쓴다.
...
때문이다. **시험 조각에서 두 값만 재면 발표된 반짝임 전부를 계산으로 고칠 수
있다. 다시 렌더할 필요가 없다.**
...
## 2026-08-24 — "논문마다 백 배 갈린다" 를 철회한다

사용자: "다른 논문들을 더 찾아. 몇개 보고 100배 다르다 결론내지말고." 맞다.
세 편 보고 내린 결론이었다. 더 찾고 이미 가진 것을 끝까지 읽으니 **틀이 틀렸다.**

### 다시 본 것
  - **TAMU 2014 그림 6 을 끝까지 읽었다.** 나는 막대 두 개만 보고 넘어갔었다.
    그 그림에는 재료가 서른여섯 개 있고, 도료·테이프·플록·펠트·마커 스무 종
    넘게가 **전부 정반사 0.2 % 아래**다. 세로축이 1 % 까지인데 오른쪽 절반이
    바닥에 붙어 있다.
  - **Nextel Velvet 811-21 규격서**: 광택도 ISO 2813 / DIN 67530 에서
    85 도 2 이하. 도료 데이터시트가 보편적으로 주는 정량값이고, 이건 정반사를
    직접 재는 규격이다. 앞으로 이 축을 쓸 수 있다.
  - **Filip 자신의 다른 재료들**: 칠판 페인트 1~3 %, 무소 0.7 % 이하.
    아크릴만 약 10 % 다. **논문끼리의 불일치가 아니라 그 논문 안에서도 outlier.**
  - 미광용 검정 도장은 **여러 겹 칠하는 것이 표준**이다. 바탕에서 오는 반사를
    막기 위해서다. 여덟 겹을 칠한 연구도 있다.

### 그래서
"논문마다 백 배 갈리므로 범위로 낼 수밖에 없다" 가 아니라
**"제대로 칠하면 1 % 아래이고, 그 하나는 도막이 얇아 바탕이 비친 것"** 이다.
Filip 논문 스스로 그 가능성을 적어 놨는데 내가 그 문장을 근거로 쓰지 않고
"표면이 다르다" 는 모호한 말로 넘겼다.

권고도 바뀐다. 발주서에 적을 것은 "값의 범위" 가 아니라 **도장 횟수와 표면
준비**다. 그걸 지키면 확산 0.99 에 들어간다.

### 방법에 대해
막대그래프를 픽셀로 재려다 두 번 실패했다(범례를 막대로 셌고, 해칭의 흰 틈에서
끊겼다). 그림을 400 dpi 로 다시 뽑아 눈으로 읽는 편이 빨랐다. **읽어서 분명한
것과 표로 주어진 값만 쓰고, 애매한 자릿수는 안 쓴다.**

### 철회했으면 등급도 고쳐야 한다
문장만 고치고 **근거 등급은 `disputed` 로 놔뒀었다.** 사용자가 물어서 알았다.
"논문이 갈린다" 를 철회했으면 그 이름을 단 딱지가 화면에 남아 있을 이유가 없다.

    wall_5pct / acryl_5pct  확산 등급  disputed -> measured

Zeng 2019 이 실제로 잰 값이고, TAMU 2014 의 스무 종과 Nextel 811-21 의 광택도가
같은 곳을 가리킨다. `source` 에 그 셋과 정정 이력을 적었다.

이제 화면에 남은 약한 등급은 `analogue`(아노다이징 거칠기, 비슷한 재료에서 빌림)
와 `guess`(무소 거칠기, 잰 자료 없음) 둘뿐이다. **둘 다 진짜로 약한 것이다.**

서버는 재료를 불러올 때 한 번만 읽으므로 다시 띄웠다. 파일만 고치고 넘어가면
2026-08-22 처럼 옛 값을 계속 내보낸다.

### "쿠폰" — 영어를 소리만 옮겨 27 군데에 퍼뜨렸다
`coupon`(시험용으로 잘라낸 작은 판)을 **쿠폰**이라고 계속 썼다. 한국어로
쿠폰은 할인권이다. 사용자: "계속 쿠폰이라는데 무슨말?"

`시험 조각` 으로 바꿨다. 문서 11 개 — NEXT, JOURNAL, SPEC_SUMMARY, QUESTIONS,
파인딩, 재료 출처, 보고서 빌더와 2 판, 절차서, 개발 여정 한국어판.
발행된 1 판은 안 건드렸다.

**소리만 한글로 옮긴 것은 번역이 아니다.** 그리고 스물몇 군데에 퍼진 뒤에
고치면 값이 비싸다. 처음 쓸 때 뜻이 통하는지 소리 내어 읽었어야 했다.

### "적분구는 총량만 준다" 도 너무 셌다
사용자가 "적분구가 머야?" 하고 물어서 설명하려다, 내가 여러 문서에 적어 놓은
주장이 틀렸다는 걸 알았다.

**적분구도 확산과 정반사를 가른다.** 정반사가 나가는 자리에 검은 마개(광택 덫)를
끼우면 그 방향으로 간 빛만 빠진다(SCE). 흰 마개로 잰 총량(SCI)에서 빼면 정반사다.
색채 측정에서 표준으로 쓰는 방식이다.

**적분구가 못 내는 것은 α 다.** 그 구멍의 크기가 정해져 있어서, 광택 덩어리가
구멍보다 좁은지 넓은지를 볼 수 없다. TAMU 가 광다이오드로 잰 것과 같은 한계다 --
꼭대기는 잡고 어깨는 놓친다.

    적분구 (SCI/SCE)  확산비율 O   α X
    고니오미터         확산비율 O   α O

고친 곳: NEXT.md, SAMPLES.md 와 그 빌더, material/_sources.json, 무소 세 종.
무소 제품 규격에 대한 말도 갈랐다 -- **장비가 못 하는 게 아니라 제조사가
마개 낀 값을 공개 안 한 것**이다.

실무에도 영향이 있다. 확산비율만 필요하면 적분구로 싸게 얻을 수 있고,
고니오미터는 α 때문에 필요하다.

### 그리고 이것도 설명 없이 쓰던 한자어였다
적분구·고니오미터·시편 — 물어보기 전까지 한 번도 안 풀어 썼다.
[[plain-korean-glossary]] 와 같은 문제다. 처음 나올 때 한 줄로 풀어 쓴다.

### 감시를 붙이며 "서버가 죽는다" 고 적었는데, 안 죽는다
사용자: "근데 가만히 나두면 서버가 죽는건가?" 재봤다.

    감시 시작 01:43 -> 마지막 점검 20:47   19 시간
    점검 통과 332 회 연속
    재시작 0 회
    서버 프로세스 10 시간 28 분째, CPU 0.0 %, 메모리 830 MB

**한 번도 안 죽었다.** 감시 스크립트 주석에 "서버가 죽거나 먹통이 되는 일이
반복됐다" 고 적었는데 그건 확인 안 하고 쓴 문장이다.

실제 기록에 있는 것은 다른 사건들이다: **배치 작업**이 Cycles Metal 셰이더
캐시 때문에 조용히 죽은 것(서버가 아니라 별도 블렌더), **작업 대기열**이
8 시간 멈춘 것, 그리고 오늘 내가 코드를 고쳐서 여러 번 재시작한 것.
**남의 고장을 서버 이력에 옮겨 적었다.**

재시작 경로는 남긴다 -- 비용이 0 이고, 정말 먹통이 되면 달리 알 길이 없다.
다만 주석을 사실대로 고쳤다: 그건 경험이 아니라 보험이다.
검사 부분은 값을 했다. 오늘 열 번 넘게 고칠 때마다 3 분 안에 회귀 없음을 확인했다.
