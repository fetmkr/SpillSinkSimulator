...
## 2026-08-25 — 업체가 못 한다고 한 것이 대부분 안 해도 되는 것이었다

정리본은 `results/FINDINGS_coating_depth_2026_08_25.md`. 여기에는 어떻게
거기까지 갔는지만 적는다.

시작은 업체 답변 한 줄이었다. "셀 9.53 에 깊이 40 은 안쪽까지 못 칠한다,
아노다이징도 안 된다." 나는 곧장 칠하는 방법을 찾으러 갔다 -- 담그기,
전착도장, 포일을 먼저 칠하기. 그건 맞는 방향이었지만 **먼저 물었어야 할
질문을 건너뛴 것**이었다. 얼마나 깊이 칠해야 하는지 우리는 안 재봤다.

32가지 연구를 다시 보니 답의 절반이 이미 있었다. 무소를 5 mm 넣으나 15 mm
넣으나 정면 총량이 0.12142 와 0.12114 로 같았다. 다만 그 연구는 깊이를 네
점에서만 봤다. 촘촘히 재보니 **1 mm 에서 끝났다.** 그리고 20 에서 40 사이에
큰 계단이 하나 있었는데, 그게 벽이 아니라 **바닥판**이었다. 바닥판은 관 속이
아니라 평평한 받침판이다. 벌집을 붙이기 전에 따로 칠하면 된다.

그러니까 정면에 대해서는 **관 안쪽을 칠할 일이 애초에 없었다.** 테두리 51 %
와 바닥판 48 %, 둘 다 드러난 평면이고, 그 사이 벽은 0.4 % 였다.

### 사용자가 물은 것이 방향을 두 번 바꿨다

"정면이 더 떨어지는게 이해가 안돼" -- 표에서 제일 이상한 값이었는데 나는
그냥 지나쳤다. 물어봐서 다시 보니 관은 한쪽으로만 열린 덫이었다. 곧게 들어온
빛은 벽을 안 스치고 바닥까지 가지만 거기서 출구가 반각 6.8 도다. 비스듬한
빛은 5.7 mm 에서 벽에 닿고 거기 출구는 40 도다. 종이 위 계산이 렌더와 2 %,
6 % 안에서 맞았다. **설명이 이야기가 아니라 계산이 됐다.** 그림도 그렸다.

"사각 셀은 어때? 격자로 만들어 미리 칠하고 붙이면 되니까" -- 이건 내가
보고서에 여섯 번째 방법으로 한 줄 적어 놓고 재보지 않은 것이었다.

### 예상이 두 번 틀렸다

사각 격자는 벽이 홑겹이라 테두리가 `2t/p`, 펼친 벌집은 붙인 자리가 두 겹이라
`3t/p`. 그래서 사각이 이길 거라고 예상했다. 정면만 맞았고 비스듬한 각도는
반대였다. 두 번째 예상 -- 90 도 모서리가 코너 리플렉터라 사각이 손해일 것 --
도 근거가 없었다.

가르는 방법을 찾는 데 시간이 걸렸다. `honeycomb` 가족이 홑겹 육각이라
테두리가 사각과 똑같다는 것(앱이 둘 다 1.68 % 로 찍는다)을 보고 나서야
변수를 하나만 남길 수 있었다. 구멍 넓이까지 맞추니 사각과 육각이 3 % 안에서
같았다. **모양은 상관이 없었다. 두 겹 벽이 전부였다.**

이건 기억해 둘 만한 방법이다. 모양 두 개를 비교할 때 "이 모양이 더 낫다" 는
결론은, 다른 게 다 같을 때만 모양 얘기다.

### 앱이 답을 막고 있었다

사각 격자를 고르면 "print floor 0.40 mm — BELOW FLOOR" 가 떴다. 0.08 mm
금속 날이 문제일 리가 없는데. 같은 표에 `"sheet, grid"` 가 한계 0.05 mm 로
이미 있었다. 모양이 공정을 정하고 있었던 것이다. 이건 전에 사용자가 지적한
것과 같은 종류다 -- 메뉴로 금지하기.

색도 같은 얘기였다. 자리마다 색이 고정이라 "어느 자리냐" 만 답하고 "무엇으로
만들었냐" 는 못 답했다. 재료 색으로 바꾸면서 한 가지를 배웠다: 라이브러리의
모든 재료가 색상 217~224 도의 검정에 가까운 파랑-회색이다. 그대로 칠하면 셋
다 까맣다. 그래서 색상은 두고 밝기만 올렸다. 그러고도 두 재료가 같아 보이면
**알려 주되 막지는 않는다.**

### 오늘 잡힌 내 실수들

- `measure()` 는 `[면][각도]` 인데 뒤집어 썼다.
- 반짝임을 -40/+40 도에서 부르고 키 이름도 틀리게 읽었다. 한 줄에 둘.
- 보고서에서 `%s` 를 안 치환해 스타일시트가 통째로 빠졌다. **숫자 검사도
  태그 검사도 통과했다.** 눈으로만 잡혔다.
- 어제 넣은 H1 검사가 내가 `addEventListener('change', note)` 를 또 쓴 것을
  잡았다. 어제 만든 검사가 오늘 값을 했다.

### 재료 파일이 옛 탭에 덮어써졌다

색을 되돌린 직후 파일이 다시 바뀌었다. 범인은 열려 있던 옛 탭이었다.
그 탭의 `CO` 가 옛 값을 들고 있다가 다시 저장했다. `/api/material_color` 는
확인 없이 덮어쓴다. 탭을 닫아서 정리했지만 **고쳐야 할 자리다.**

---

## 2026-08-25 저녁 — 구조를 파악하고 렌더 캐시를 지웠다

사용자가 "폴더랑 파일 너무 퍼져 있는데 그 구조 한번 파악해" 라고 했다.
세어 보니 퍼진 것은 **배치가 아니라 용량**이었다.

### 옮기지 말라는 답이 나온 이유

`scripts/*.py` 경로가 문서·코드·보고서 **488 곳**에서 인용된다 (서로 다른 파일
185 개). `results/` 경로가 180 곳, FINDINGS 를 가리키는 파일이 59 개.
폴더를 나누면 그게 다 깨진다. **지금 배치는 비용이 붙어 있는 배치다.**
필요한 것은 재배치가 아니라 지도였고, `START_HERE.md` 7 절에 넣었다.

### "되돌릴 수 없다" 를 뭉뚱그린 것

정리 후보 셋을 묶어서 "지우면 되돌릴 수 없으니 확인받겠다" 고 했더니
사용자가 **"되돌릴수 없는게 머야?"** 라고 물었다. 맞는 지적이었다. 셋의
성격이 완전히 달랐고 나는 확인 없이 한 문장으로 묶었다. 세어 보니

- **맥 사본 251 개** — 238 개는 원본과 바이트까지 같고, 다른 12 개는 전부
  원본보다 오래됐다. 원본보다 최신인 것 **0 개**. 잃을 것이 없다.
  (예전 메모에 "155 개, 149 개 동일" 이라 적혀 있었는데 그 사이 96 개가
  늘어 있었다. 오래된 확인을 그대로 쓸 뻔했다.)
- **보고서 갤러리** — 보고서 HTML 이 `shots/` 를 가리키는 곳이 **0 건**,
  그림은 전부 base64 로 품고 있다 (2026-08-15 는 78 장). 원본 해상도만 잃는다.
- **renders/** — 이것만 진짜 대가가 있었다. 옛 숫자를 프레임으로 다시 따지는
  길이다.

### 그 대가가 이미 반쯤 없었다

옛 CSV 가 적는 것은 tag·family·topology·seed·diffuse_frac·roughness·theta·
rho·control·params_json 뿐이다. **표본 수·해상도·빔 폭·여백·블렌더 판이
없다.** 프레임으로 돌아가도 그 조건을 모르니 똑같이 다시 렌더할 수 없다.
이건 이 프로젝트의 자체 동료검토가 이미 적어 둔 것이다
(`results/PEER_REVIEW.md:505`).

그리고 결정적인 것: **`results/*.md` 45 편 전체에서 옛 EXR 을 다시 열어 뭘
알아냈다는 기록이 0 건이다.** 2026-08-20 리그 감사도 옛 프레임을 읽은 게
아니라 고친 리그로 다시 렌더해서 알아냈다. 그럴 만하다 -- 옛 프레임은 창·
해상도·빔 폭이 틀린 리그로 찍혔으니, 열어 봐야 망가진 리그가 뭘 봤나만 나온다.

### 지우기 전에 목록

134,094 개 · 33.0 GB. 크기·수정시각·경로를 전부 적어
`results/renders_manifest_2026-08-25.txt.gz` (0.9 MB) 에, 폴더별 요약을
`.md` 에 남겼다. 몇 MB 로 "그때 뭘 렌더했나" 는 답할 수 있게 된다.

지운 뒤 **말로 안전하다고 하지 않고 실제로 돌렸다.** 점검 45 항목이 발표된
32 가지 재현(D 그룹, 진짜 렌더한다)까지 포함해 전부 통과했고,
`renders/form_b` 가 자동으로 다시 생겼다. 디스크 여유 25 GB → **55 GB.**

보고서 갤러리와 맥 사본은 사용자가 "렌더만" 이라고 해서 안 건드렸다.

### 하나 더 알게 된 것

`renders/form_b` 는 캐시가 아니라 **찌꺼기**다. `form_buildable.py` 만 쓰고
아무도 다시 안 읽는다 (숫자는 `results/form_buildable.json` 으로 간다).
그런데 청소를 안 해서 계속 큰다 -- **오늘 하루에만 9,360 개가 쌓였다.**
7 GB 중 상당수가 그것이었다. 주기적으로 비우는 것이 맞다.

---

## 2026-08-25 오후 — 전파를 보러 갔다가, 우리 자료가 가리키던 곳으로 돌아왔다

사용자가 물었다. "rf 최신 연구 논문 좀 찾아봐. 어떤 형상이 전파를 흡수 반사
잘하는지." 그리고 이어서 "그래서 우리가 먼가 새로운 모양이나 새로운 방법을
이걸 바탕으로 생각한다면?"

전파 쪽은 볼 게 많았다. 무반향실 피라미드의 설계 규칙이 파장으로 딱 정해져
있고, 요즘은 3D 프린트로 기울기 임피던스 구조를 만든다. 코딩 메타표면은
반사파를 여러 방향으로 쪼갠다.

**그런데 대부분이 안 넘어온다.** 파장을 계산해 보고 알았다. 전파 10 GHz 는
파장 30 mm 라 구조와 같은 크기지만, 우리 445 nm 빛에게 셀 9.53 mm 는
**파장의 21,415 배**다. 저쪽은 파동 문제고 우리는 광선 문제다. 임피던스
정합도 위상 트릭도 저항 잉크도 파장 규모 구조를 요구한다.

남은 것은 원리 하나였다. **갑자기 바꾸지 말고 서서히 바꾼다.** 우리 말로
옮기면 살 두께를 깊이에 따라 바꾸는 것이다.

그러고 나서 우리 아침 자료를 다시 봤다. 테두리 51.4 %, 벽 0.4 %, 바닥판
48.3 %. 깊이로 못 없애는 두 면. **새 아이디어는 전파에서 온 게 아니라
우리 표에 이미 적혀 있었다.** 전파는 어느 방향으로 갈지만 알려 줬다.

### 두 가지가 됐다

`wall_top` 이 노출 넓이를 혼자 정한다는 것(`geom_topo.py:248`)을 보고
**테두리만 깎았다.** 강도는 아래벽이 정하니 위만 얇게 하면 공짜다.
0.08 에서 0.03 으로 정면이 31 % 준다.

**바닥판을 피라미드로 바꾼 것**이 제일 컸다. 반짝임이 0.4280 에서 0.0639 로
6.7 배 떨어졌다. 정면으로 관을 들여다볼 때 평평한 판 대신 기울어진 면을
보기 때문이다. 만들기도 어렵지 않다 -- 평판 대신 눌러 찍은 판을 깔면 된다.

그리고 **모양 뭉개기가 처음으로 움직였다.** 0.9594 에서 1.0363. 깊이도
셀 크기도 포일 두께도 셀 모양도 못 움직이던 축이다.

### 뒤집힌 피라미드 -- 내가 또 예상으로 말했다

사용자가 물었다. "누가 역으로 뒤집힌 피라미드 말하는데. 지금 피라미드의
몰드 형태 말이야."

찾아보니 태양전지 쪽이 이미 비교해 놨고, 이유가 우리 계산과 같아 보였다 --
파인 홈에서 빛이 한두 번 더 튕긴다. 실리콘 실측으로 선 피라미드 12~14 %,
뒤집힌 것 7 % 미만. **나는 그 자리에서 "남들이 이미 쟀습니다, 뒤집힌 쪽이
낫습니다" 라고 했다.** 논문은 맞았고 내 적용이 틀렸다.

만들어 재보니 총량과 반짝임은 사실상 같고 **뭉개기가 1.46 에서 1.07 로
무너졌다.** 40 도에서 되돌아온 봉우리가 4.4 배였다. 파인 홈은 직각 두 면이
코너 리플렉터가 되어 빛을 온 쪽으로 되돌려 보낸다. **태양전지에서 좋은
이유가 정확히 그것이다** -- 되돌아온 빛이 실리콘 안으로 다시 들어가 흡수된다.
같은 성질이 저쪽엔 이득, 우리한텐 손해.

오늘만 두 번째다. 아침에는 사각 격자가 테두리 때문에 이길 거라고 했고
비스듬한 각도에서 반대였다. **다른 분야의 결론은 그 분야의 조건과 함께
온다. 조건을 안 옮기고 결론만 옮기면 틀린다.**

### 검사기가 또 값을 했다

`pyramid_inv` 첫 판은 열린 모서리가 9,604 개였다. 셀마다 꼭짓점을 따로
만들어 옆 셀과 안 붙었던 것이다 -- 위 평면이 통째로 체였다.
`gate_model_fitness` 가 첫 실행에서 잡았고 (선 피라미드는 같은 표에서 0),
고친 뒤에 숫자를 냈다. **모양을 만들면 숫자를 내기 전에 검사부터 돌린다.**

---

## 2026-08-27 — 방을 알고 나니 우리가 잰 축이 틀렸다

보고서: `report/comb/room_and_observer_2026-08-27.html` (한/영),
`report/comb/pyramid_height_2026-08-27.html`
훑기: `scripts/sweep_observer_scan.py` → `results/comb20/observer_scan_both.json`

### 두 주 동안 이 방에서 안 쓰는 숫자를 재고 있었다

사용자가 설치 조건을 알려줬다. 방 10 x 10 m, 천장 6 m, 프로젝터가 높이 2 m
에서 수평보다 45~60 도 **위로** 쏜다. 패널은 **천장**에 붙는다.

    쏘는 각 45 도 -> 패널에 45 도
    쏘는 각 60 도 -> 패널에 30 도

**정면은 이 방에서 안 일어난다.** 정면이 되려면 프로젝터가 천장 바로 밑에
있어야 하는데 그러면 빔이 위로 안 가고 공중 이미지가 안 생긴다.

그런데 이 프로젝트가 발표한 "정면 반짝임" 은 전부 판 법선 방향 값이었다.
`setup_camera` 는 처음부터 `elev_deg` 를 받았는데 `run_case` 가 0.0 을 글자
그대로 넘겼다. 벌집 정면 2.598 이 무서워서 벌집을 접었던 것도, 피라미드 정면이
좋다고 좋아했던 것도 이 방과 상관없는 이야기였다.

### 관찰자 각도를 손잡이로 만들었다

`form_buildable.OBS_ELEV`, `/api/form` 의 `obs_elev`, 화면의 칸, 그리고 3D
뷰에 카메라 그림. 안 보내면 판 법선이라 발표된 66,426 행은 안 움직인다.

두 가지가 딸려 나왔다. (a) 기울여 보면 세로가 `cos` 만큼 눌리므로 그 축의 mm
환산을 `1/cos` 로 고쳤다. 안 고쳤으면 비스듬히 본 뭉개기가 전부 좁게 읽혔다.
(b) `form()` 이 돌려주던 봉우리가 **항상 θ=0 값**이라 40 도 빔의 봉우리를 물을
방법이 없었다. `peak_by_theta` 를 더했다.

검사기 `gate_observer_angle.py` 4항목. 허용 폭 1e-6 은 짐작이 아니라 실측이다 --
아무것도 안 바꾸고 같은 요청을 세 번 보내면 봉우리가 상대 3.1e-08 갈린다.
처음에 1e-9 로 잡았다가 렌더러 흔들림을 내 변경으로 오해할 뻔했다.
그리고 이 검사기가 **디스패치 람다에서 `obs_elev` 가 빠진 것**을 잡았다
(C 항목이 0.0 % 움직임으로 실패). 이 프로젝트가 세 번째 당하는 결함 종류다.

### 표의 반쪽만 읽고 정반대로 발표할 뻔했다

첫 훑기 결과에 `+40` 과 `-40` 두 칸이 있었는데 `-40` 만 보고 "관객 자리는
어둡다 (0.0068)" 고 썼다. `+40` 이 **20 배 밝다.**

부호가 뭘 뜻하는지는 코드에 있었다. `add_stripe` 는 램프를
`(cx, d cos t, z + d sin t)` 에 놓고 `setup_camera` 는 카메라를 **글자 그대로
같은 식**으로 놓는다. 그러니 빔 +40 에 관찰자 +40 은 빛이 온 쪽에서 되보는
것이고, 빔 -40 에 관찰자 +40 은 거울 방향이다. 거울 방향이 오히려 어둡다.

### 모양은 빛을 없애지 않고 옮긴다

빔 40 도, 둘 다 무소 팁 20 mm 에 나머지 5 % 페인트. 민판 무광 검정 = 1.0.

| 관찰자 | 벌집 9.53/40 | 피라미드 50/250 | 어느 쪽이 어두운가 |
|---|---|---|---|
| 0도 | 0.0070 | 0.0773 | 벌집 11배 |
| 20도 | 0.1329 | 0.1001 | 피라미드 1.3배 |
| 30도 | 0.1347 | 0.1585 | 벌집 1.2배 |
| 40도 | 0.1367 | **0.4675** | 벌집 3.4배 |
| 50도 | 0.1365 | 0.1124 | 피라미드 1.2배 |
| 60도 | 0.1344 | 0.0670 | 피라미드 2.0배 |

반사 총량으로 보면 벌집이 민판보다 42 배 어둡다. 그런데 빛이 온 쪽에서는
0.137 이다. **총량이 줄어도 특정 방향에는 몰린다.**

**벌집이 밝은 이유는 관이 아니라 벽이다.** 빔이 40 도로 들어오면 셀 한쪽 벽이
`간격/tan40` = 11.4 mm 까지 밝아지고, 그 면의 법선은 관 축과 직각이다.
관 축을 똑바로 내려다보면 벽이 옆으로 서서 안 보이고 어두운 바닥만 보인다.
빛이 온 쪽에서 보면 밝아진 벽이 그대로 보인다. 반대쪽에서는 그 벽의 뒷면이다.
그래서 좁은 봉우리가 아니라 **20~60 도가 통째로 평평하게 밝다.**

셈이 딱 맞는다. 깊이 11.4 mm 인 밝은 띠 끝에서 40 도로 올려다보면 가로로
9.53 mm 를 가는데 그것이 셀 너비다. **밝은 띠 전체가 그 방향에서만 다 보인다.**

**피라미드는 골이 되쏜다.** 밑변 50 에 높이 250 이면 옆면이 바닥에서 84 도로
거의 서 있다. 마주 보는 두 면이 좁은 골을 이루니 두 번 튕긴 빛이 온 길로
되돌아 나간다. 40 도에서 0.468 이고 60 도로 벗어나면 7 배 떨어진다.
[[pits-retroreflect-peaks-scatter]] 와 같은 물리인데 이번엔 선 피라미드다.

여기서 불편한 짝이 생긴다. **높이를 올릴수록 총량은 좋아지는데 골은 더
좁아진다.** 두 축이 반대로 간다.

### 그래서 답이 하나가 아니다

관객이 프로젝터 가까이 서면 피라미드, 프로젝터 방향이면 벌집, 멀리 떨어져
60 도로 올려다보면 피라미드다. **다음에 정할 것은 모양이 아니라 관객 자리다.**

그리고 **둘 다 목표를 못 넘었다.** 목표 0.040 인데 프로젝터 쪽 바닥에서
벌집 3.4 배, 피라미드 11.7 배다.

### API 가 버린 값을 조용히 쓰고 있었다

`/api/measure` 가 확산을 안 받으면 버린 값 0.76 을, 거칠기는 버린 0.30 을
썼다. 25 % 틀린 숫자가 아무 표시 없이 나왔다. `/api/rays` 도 같았다.

처음에 세 번을 임기응변으로 고쳤고 사용자가 멈춰 세웠다. 원칙대로 다시 했다 --
결함 종류 전체를 훑고, 규칙을 적고, 검사기를 만들고, 옛 코드에서 실패하는 것을
확인하고, 고쳤다. **그 검사기가 내가 못 찾은 네 번째 자리를 잡았다.**
`gate_api_defaults.py` 6항목. 화면과 배치 스크립트는 값을 명시해 보내므로
발표된 숫자는 무사하다.

### 보고서가 서식이 안 풀린 채로는 안 나간다

파이썬 서식 문자열 밖의 `%%` 는 안 줄어들어 화면에 두 개가 찍히고, 안 끼워진
`%s` 는 문단을 통째로 날린다. 이 실수를 네 번 했다. 숫자 검사도 태그 검사도
못 잡는다 -- 태그는 다 맞고 숫자도 다 맞기 때문이다.

이제 `build_room_report.py` 와 `build_pyramid_height_report.py` 는 파일을
**쓰기 전에** 본다. 옛 결함으로 되돌려 실제로 발행이 막히는 것까지 확인했다.
그리고 그날 아침 발행한 피라미드 보고서에도 세 군데 남아 있었다.

### 그림을 세 번 다시 그렸다

방 단면 첫 판은 글자가 서로 겹쳐서 못 읽었다. 각도 글자를 오른쪽 빈 칸에
세우고 가는 안내선으로 닿는 자리를 가리키게 바꿨다. 셀 단면 첫 판은 눈과
숫자가 그림 위로 잘려 나갔고, 밝은 면을 벽 두 개에 칠해서 옆 셀 이야기를
끌어들였고, 반대쪽 눈이 다른 셀을 가리키고 있었다. **셋 다 브라우저로 열어
보고서야 보였다.** [[render-geometry-before-measuring]] 이 그림에도 그대로다.

## 2026-09-14 ~ 15 — 감사를 받고 장비를 고쳤다. 그리고 내가 세 번 혼났다

### 왜 이 일을 했나

외부 감사(`results/FINDINGS_simulator_audit_2026_09_14.md`)가 결함 다섯 개를 짚었다.
요청 전달 두 갈래, 상반성 어긋난 코팅, 무소 논문 오독, 화소 하나짜리 반짝임 통계,
거짓 수렴. 다섯 다 그동안 낸 숫자를 흔들 수 있어서 새 형상보다 먼저 고쳤다.
감사의 주장을 먼저 재현했다 (탐침 두 개를 다시 돌려 비트 단위로 같음, 그림 6 을 500 dpi 로 잘라 TIS 0.96 확인).

### 어떻게 고쳤나

- 첫날(14일)은 문제 파악까지만 하고 커밋했다. 사용자가 "코드 작성하지말고" 문제부터 보라고 했다.
- 무소 맞춤이 3 축 격자라 너무 느렸다. 모델이 확산·광택 비율에 선형이라 거칠기 한 축만 훑게 바꿔 25 초.
- Cycles Principled 가 Schlick 이 아니라 정확한 프레넬을 쓴다는 것을 렌더로 확인했다 (Schlick 로 보면 14 % 어긋남).
- 기준 시료 4/22: 트리만 바꾸면 0.4 % 안, 재료를 바꾸면 총량 1.7 배·정면 box 2.1 배·뭉개기 −14 %.

### 혼난 것 셋

1. **비용을 이유로 일을 뺐다.** 광선 그림, 빛줄기 수 검사, 목표 0.040 재측정, 과거 결과를 "나중" 으로 미뤘다.
   사용자: "비용따지지 말고 해". 다 했다. 빛줄기 수 검사를 다시 짜 보니 16 이 box 에 모자랐다 (+4.55 %) → 256.
   미룬 일에서 진짜 결함이 나온 셈이다.
2. **알림에 답하다 영어로 넘어갔다.** "왜 영어로 말하지?"
3. **말을 잘못 알아듣고 크게 벌였다.** 사용자는 "랜덤하게 2개 뽑아서 더해보고 결론 내리자" 고 했다.
   보고서 재현을 후보 전부 말고 무작위 2 개만 하자는 뜻이었다. 나는 렌더 난수 설정을 바꿔 다시 재자는 뜻으로
   바꿔 선택지를 만들어 물었고, 서버·화면에 칸을 넣고, 14 개를 다시 재는 체인을 걸고, 보고서 순위 규칙까지 바꿨다.
   사용자는 그 용어가 무슨 뜻인지도 모른다고 했다. 전부 되돌렸다. 되돌린 뒤에도 채팅에서 그 용어를 또 써서 혼났다. 선택지를 내 해석으로 만들면 사용자가 고른 답도 내 해석이 된다.

### 1,643 개 대신 14 개

전체 재산출을 256 빛줄기로 다시 시작했다가, 사용자가 "합의한 구조와 콤비네이션만 하면 되는 거 아니냐" 고 했다.
큰 순위는 이미 나와 있었다. 14 개만 쟀다. 재산출은 12 개에서 멈춰 있고 돌리지 말라고 했다.

### 시뮬레이터가 보고서와 같은 답을 내게

사용자가 보고서 값을 화면에서 직접 확인하겠다고 했다. 대조해 보니 화면과 보고서 사이에 어긋남이 여럿이었다.
각도, 방위, 빛줄기 수, 판을 키운 값을 먼저 돌려주던 것, 칠 깊이 % 반올림, V 홈 프리셋의 잔 홈, 바닥판 칸.
전부 한 곳(`form_metrics`)에서 읽게 맞추고 바닥판 칸이 달랐던 6 행을 다시 쟀다 (차이 0.23 % 이하).
설계도는 처음에 깊이를 6.4 로 그렸다 (실제 22). 가장 깊은 단면을 찾게 고쳐 14 개 모두 설계값 2 % 안.

### 켜 둔 서버로 다시 렌더한 대조

차례대로 5 행을 다시 렌더하니 보고서와 0.07 % 이하로 맞았다. 같은 조건이라 같은 빛줄기를 다시 쏜 것이고,
화면 경로가 보고서 경로와 같다는 확인이다. 나머지 9 개 중 무작위로 뽑은 2 개(V 홈 13, 밑변 50/250)만
더 재고 끝낸다. 보고서에 "시뮬레이터 검증" 절을 넣었다.

## 2026-09-16 — 벌집을 다시 보다가 형상 코드 결함을 찾았다. 내가 다섯 번 틀렸다

정본: `results/FINDINGS_comb_alu_cell_2026_09_16.md`.
보고서에 8 절 "덧붙임 · 9월 16일" 을 붙였다.

### 어떻게 시작했나

사용자가 내일 조사할 것을 물었다. 벌집 가격, 직접 만드는 법, 그리고 "은색 벌집인데
위·아래·옆면만 칠하고 셀 내부는 안 칠한 것" 시뮬레이션.

먼저 확인해 보니 **2026-08-25 의 칠 깊이 측정은 안 칠한 부분을 5 % 무광 검정으로
놓고 한 것**이었고, 그 문서 9 절에 이렇게 적혀 있었다.

> "관 안쪽에 **아무것도** 못 넣는 경우. 맨 알루미늄 값이 재료표에 없고, 없는 재료로
> 숫자를 내지 않는다."

**사용자가 물은 것이 정확히 이 프로젝트가 안 잰 것으로 남겨둔 항목이었다.**

### 재료부터

Pozzobon & Levasseur 2019 (Biotechnology Reports 25:e00399) 를 찾았다. 적분구로
잰 가정용 알루미늄 포일의 가시광 총반사가 **86 %, 밝은 면과 무광 면이 같다.**
정반사는 밝은 면이 높고 확산은 무광 면이 높다고만 적혀 있고 **비율의 수치는 없다**
(원문 유료, 자료 저장소에도 없음). 그래서 양 끝을 잡으려고 셋을 만들었다.
...
이 도료 모델은 유전체 프레넬이라 정면 정반사가 최대 4 % 다. 총반사 86 % 를 담으려면
확산이 0.953 이상이어야 한다. **시뮬레이터에 금속 모델이 없다.** 내가 재료 파일에
"금속 주의" 라고 적어둔 것을 검사기가 그대로 잡았다. 확산 쪽 끝만 돌렸고, 그쪽이
우리에게 불리한 끝이라 결론은 선다.

### 답 — 위 20 mm 만 칠하면 된다

테두리만 칠하면 30 도 26 %, 반짝임 18.41. 무너진다.
위 20 mm 칠하면 세 축이 전부 칠한 것과 같다 (0.166 % / 1.014 / 0.1215).

셈과 맞는다. 비스듬한 빛이 벽에 처음 닿는 깊이가 45 도 4.8 mm, 30 도 8.3 mm 다.
첫 튕김이 칠한 구간에 떨어지면 거기서 죽는다.

**뒤집어서도 확인했다.** 위를 안 칠하고 **아래 20 mm 만** 칠하면 30.34 % — 아예
안 칠한 것(31.5 %)과 4 % 차이다. **아래쪽 도료는 아무 일도 안 한다.**
설명만 하고 넘어갈 뻔했는데 사용자가 "이게 말이 되나" 라고 물어서 검사 셋을 돌렸다.

### 사용자의 질문 넷이 네 번 방향을 바꿨다

**"0 도로, 혹은 3 도 5 도로 들어가면?"** — 구멍이 있었다. 첫 벽 닿는 깊이가
`(셀/2)/tan(입사각)` 이라 **11~13 도에서 21~25 mm**, 칠한 20 mm 를 지나친다.
봉우리 6.4 배. 금지 구간 4~23 도. 방 조건 30~45 도는 안전하고 여유 7 도.
30 mm 칠하면 사라진다. **내가 "20 mm 면 된다" 고 한 것은 30/40/45 도만 보고 한 말이었다.**

**"이러면 두께가 20 미리면 되는 거 아니야?"** — 맞았다. 깊이 40 → 20 이 방 조건에서
6.7 % 차이다. **깊이 40 은 정면 축(0.039 대 0.080)으로 고른 것이고, 정면 축은 이틀 뒤
2026-08-27 에 폐기됐는데 깊이 결정을 다시 안 봤다.** `room_and_observer` 에서 배운
것을 여기서 한 번 더 놓치고 있었다.

**"셀 크기가 큰 게 어둡다고?? 45 도면 바닥을 치고 나올 텐데"** — 이것도 맞았다.
내가 "방 조건 최대" 한 칸만 보고 "셀이 클수록 어둡다" 고 했다. 그 값이 45 도라
바닥 효과가 안 보였다. 각도별로 다시 보니 **45 도 한 칸만 큰 셀이 이기고 나머지는
다 진다.** 바닥 재료를 바꿔보니 30 도가 셀 6.5 에서 +0.1 %, 셀 20 에서 **+135.7 %**
움직였다. **큰 셀은 바닥판이 그대로 보인다.**

**"빛 들어가는 면도 돌려가면서 했어? 45 도 90 도?"** — 셀 훑기를 방위 0 하나로만
했다. 돌려보니 육각은 60 도 되풀이라 0 도와 30/90 도가 양 끝이고, **규약의 방위 45 는
한가운데(15 도와 같음)라 꼭짓점 끝을 한 번도 안 본다.** 놓치는 몫은 1 % 안이라 규약은
그대로 둔다. 다만 **판 까는 방향이 8 % 를 정한다.**

### 그러다 나온 것 — `comb` 형상 코드가 pitch 9.53 에서 다른 격자를 짓는다

셀 크기 훑기에서 9.53 만 줄에서 벗어났다. 지금 쓰는 값이 마침 제일 좋다는 것이
의심스러워 이웃을 쟀다.

    pitch 9.529999   꼭짓점 28,736   겉넓이   984,253   40 도 0.1963 %
    pitch 9.53       꼭짓점 32,792   겉넓이 1,099,534   40 도 0.1689 %
    pitch 9.5300001  꼭짓점 28,736   겉넓이   984,253   40 도 0.1963 %

**1천만분의 1 mm 로 벽이 12 % 늘고 값이 14 % 어두워진다.** 6.35 도 그렇고 6.5 와
12.7 은 멀쩡하다. 캐시가 아니고(로그에 `[RESULT]` 가 새로 찍힌다) 격자 검사도 통과한다.

**발표된 벌집 총량이 9~10 % 움직인다.** 벌집 9.53/40 이 0.208 → 0.2285,
전체 2 위 조합이 0.197 → 0.2149. 원뿔 0.223 에게 진다.

**어느 쪽이 맞는지 아직 모른다.** `_build_comb` 주석에 실제 펼친 벌집의 두 겹 벽을
모델링한다고 적혀 있어, 9.53 만 제대로 짓는 것일 수도 있다. 증거는 "9.53 이 틀렸다"
쪽이다 — 멀쩡한 다섯 점이 매끄러운 줄을 이루는데 의심 두 값만 그 줄 아래로 벗어난다.

### 오늘 내가 틀린 것 다섯

1. **`honeycomb` 갈래를 지어놓고 `comb` 과 같다고 했다.** 다른 물건이다.
   요청이 쓰는 것은 `topology="comb"` 이다.
2. **`margin_depths` 가 렌더에서 2.0 으로 덮어써지는 것을 몰랐다.** 요청의 0.2 는
   안 쓰인다 (`_render_params`). 그것도 모르고 형상을 비교했다.
3. **"셀이 클수록 어둡다."** 한 칸만 봤다.
4. **"20 mm 면 된다."** 방 각도만 봤다.
5. **셀 훑기를 방위 0 하나로만 했다.**

**다섯 다 사용자의 질문이나 반박에서 나왔다.** 1 과 2 는 내가 "형상이 같다" 고
단정한 뒤에 드러났다. **읽고 짐작하지 말고 지어서 재야 한다는 것을 또 배웠다.**

### 한 줄로

**축을 바꾸면, 그 축으로 내렸던 결정을 전부 다시 봐야 한다.** 깊이 40 이 그 예다.
그리고 **한 칸만 보면 다른 칸의 문제를 숨긴다.** 셀 크기가 그 예다.

---

## 2026-09-17 — 시연 준비. 서버를 막았고, 근거를 부풀렸고, 영어로 보고했다

사용자가 벌집으로 결론 내고 연구자에게 30 분 시연하기로 했다. 진행표를 만들다가
화면을 직접 눌러 보니 **측정은 전부 맞고 화면이 틀린 곳투성이**였다. 바닥판 "같게" 가
bare 재료를 따라가서 아크릴을 고르면 바닥판이 몰래 바뀌었고, 결과 밑 재질 줄은
5 % 민판을 무소라고 적었고, 단추 설명은 옛 규약(5 각도 3 평면, 6 위치 90 초)이었고,
결과에는 관객 각도가 안 찍혔고, 광선 추적에는 방 조건 30·45 도가 없었다. 고쳤다.
화면 요청 검사는 14/14 그대로다.

### 오늘 내가 틀린 것

1. **진행표용 뭉개기 측정(판 500)을 사용자가 쓰는 서버로 보냈다.** 서버는 한 번에
   하나만 돌고 취소가 없다. 사용자가 "렌더 토털 눌렀는데 멈춰버림" 이라고 했다.
   내 요청을 죽이고 서버를 다시 켰다. **긴 측정은 별도 프로세스로.**
2. **"가장자리를 안 읽는 기록된 이유가 세 가지" 라고 했다.** 사용자가 "근거는 머야?"
   하고 물어서 원문을 보니 기록된 실측은 하나(배경 섞임)뿐이었다. 하나는 내 추측,
   하나는 반짝임 쪽 설정을 총량에 갖다 붙인 것이었다.
3. **보고를 영어 문장으로 썼다.** "한글로 말해" 를 들었다.
4. **bare 칸을 잠그자고 제안했다.** "니맘대로 잠그고" 라고 혼났다. 사용자가 원한 것은
   고른 값이 보이는 것이었다. 시킨 문제(보이는 값)만 고쳤어야 했다.
5. **9/16 FINDINGS 에 "10 mm 는 형상 코드가 이상하게 구는 값이 아니다" 를 안 재고
   적었다.** 오늘 지어 보니 10 도 1e-7 에 메시가 바뀐다. 총량은 0.16 % 라 괜찮았지만,
   확인은 오늘 처음 했다.

### 한 줄로

**측정기가 맞아도 화면이 틀리면 시연에서 틀린 숫자를 말하게 된다.** 화면은 서버
응답을 그대로 보여줘야 한다 (재질 줄, 관객 각도 둘 다 그 원칙으로 고쳤다).

### 같은 날 이어서 — 진행표와 시연 설계

진행표(https://claude.ai/artifact/H6ywiNQ91g7MhCd337zhNR)를 만들고, 화면 설계 목록에
진행표 번호대로 여섯 설계를 넣었다. 여섯 다 목록에서 불러와 눌러 보다가 **판 500 설계가
200 으로 재지는 결함**을 찾았다. 범위 슬라이더는 최대값에서 멈추는데, 불러오는 길이
범위를 안 넓혔다. 셀 10 이 0.22745 가 아니라 0.22785 로 나와서 알았다. 아는 답과
견주지 않았으면 진행표에 판 200 값을 판 500 이라고 적을 뻔했다.

결론 벌집 세 축이 찼다: 총량 0.227 %, 반짝임 0.1453, 뭉개기 1.03~1.10 (관객 20~60).
읽는 창 50×50 으로 바꿔도 모든 칸 0.18 % 안.

---

## 2026-09-18 — 천장이 3.5 m 였다. 그리고 내가 문서 레이아웃을 두 번 깼다

사용자가 "천장 높이가 3.5 m 야" 라고 했다. 이 프로젝트의 모든 각도 조건이 천장 6 m 가정 위에 서 있었다.
계산해 보니 프로젝터 2 m 기준으로 **45 도가 되는 거리가 4.0 m 에서 1.5 m 로** 줄었다. 그보다 먼 천장은
전부 45 도를 넘는데, **우리는 0~45 도만 쟀다.**

사용자가 "정면각이 없어지니 오히려 좋은 것 아니냐" 고 했다. 반대다. 우리 설계에서 정면이 가장 어두운
각도였다 (셀 10 벌집 0° 0.075 %, 45° 0.227 %). **낮은 천장은 어두운 각도를 없애고 밝은 각도만 남긴다.**

각도를 직접 만져 보는 도구를 만들어 연구 기록 00 절에 넣었다. 처음에는 "화면(공중 영상)을 겨눈다" 로
만들었는데 사용자가 **"화면은 필요 없고 천장의 어디를 보는지"** 로 바꾸라고 했고, **프로젝터를 좌우에
하나씩 두고 겹치는 구간을 보자**고 했다. 그렇게 고치니 이야기가 됐다.

### 오늘 내가 틀린 것

1. **`</div>` 하나를 더 남겨 문서 레이아웃을 깼다.** 아홉 절 중 여덟 절이 본문 틀 밖으로 나갔다.
   사용자가 화면을 보고 "레이아웃 왜 이래?" 라고 해서 알았다. **HTML 을 잘라 붙이면 태그 균형을 세어야 한다.**
2. **어두운 화면 설정을 남겨 뒀다.** "시뮬레이터 색으로" 라고 했으면 시뮬레이터처럼 밝은 화면 하나여야 했다.
3. **로컬 미리보기와 올린 문서를 구분해서 말하지 않았다.** 그림이 깨진 것은 내 로컬 폴더에 파일이 없어서였는데,
   사용자는 올린 문서가 깨진 줄 알았다. **어느 주소를 보고 있는지 먼저 밝힌다.**
4. **화면(공중 영상) 기본값을 내가 지어냈다.** 사용자가 "니가 이렇게 설정한거야?" 라고 물어서 밝혔다.
   지어낸 값은 물어보기 전에 먼저 "이건 내가 고른 값" 이라고 적어야 한다.

### 한 줄로

**조건이 바뀌면 숫자보다 먼저 각도를 다시 본다.** 천장 높이 하나가 순위 각도 전체를 흔들었다.

### 같은 날 이어서 — 스치는 각도를 잴 수 있게 했다

사용자가 "45 도 넘는 각도 시뮬이 필요한데 시뮬레이터에 그런 옵션이 없네" 라고 했다. 맞다. 측정 요청은
각도를 받게 되어 있는데 화면에 칸이 없었다. 규약은 그대로 두고 요청에만 각도를 더하는 체크 칸을 넣었다.

재 보니 **전부 밝아졌다.** 사용자가 "왜 다들 밝아져?" 라고 물었다. 도료의 프레넬이다. 거의 완전 확산인
5 % 판은 1.05 배로 안 변하고 광택 있는 무소는 2.01 배가 된다. **구조가 진 게 아니라 도료가 오른 만큼
따라 올랐다.** 이 비교를 못 했으면 "스치는 각에서 벌집이 무너진다" 고 잘못 말할 뻔했다.

그리고 내가 "피라미드가 75 도에서 0.689 % 로 무너진다" 고 먼저 말했는데, 그 값은 **방위 0 도 한 칸**이었다.
방위 45 도에서는 0.365 % 다. 칸 하나를 설계 전체의 성질처럼 말한 것이라 바로 정정했다.
