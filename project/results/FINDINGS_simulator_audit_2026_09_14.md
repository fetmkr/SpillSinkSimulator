...
`gate_coating_reciprocity.py` (민판, 빛줄기 256, 씨앗 17), 6/6:
...
실제 측정 경로(`sim_server.form`), 빛 퍼짐 0.05 도, musou_fit2 팁 20 mm + 5 % 페인트, 위상 16, sobol, 씨앗 1·2. 모양 셋(민판 200, 벌집 9.53/40, 피라미드 50/250) x (빔, 관찰자) 넷. 참값은 256 두 씨앗 평균. 판정은 열두 경우 전부에서 참값과 1 % 안, 씨앗 차 1 % 안. 144 렌더, 54 분.

| 통계 | 충분한 빛줄기 | 16 에서 가장 크게 어긋난 경우 |
|---|---|---|
| 뭉개기 | 8 | 0.3 % 안 (전 경우) |
| p99 | 64 | 피라미드 관찰 0 +1.46 %, 벌집 정면 씨앗 차 3.8 % |
| **box** | **256** | **벌집 정면 +4.55 %**, 피라미드 관찰 60 −1.37 %, 벌집 30/30 −1.23 % (씨앗 차 1.28 %). 64 에서도 벌집 정면 +1.84 % |
| max | 256 | 벌집 정면 +11.2 % |
...
2. 사용자가 보고서 값을 화면에서 재현한다. 어긋나면 먼저 `gate_finalists_ui_path.py` 로 요청을 대조하고, 그다음 렌더 값을 본다.
3. 사용자가 정할 것: 새 반짝임 목표 (옛 0.040 은 비교 불가, 지금 기준 표준 샘플 0.077), 1,643 개 재산출을 이어 돌지.
